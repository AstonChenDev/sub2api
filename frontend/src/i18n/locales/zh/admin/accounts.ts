export default {
    accounts: {
      ...
      upstreamHTTPVersion: {
        label: '上游 HTTP 协议',
        description: '控制该账号连接 OpenAI 兼容模型上游时使用的 HTTP 协议。',
        auto: '自动（继承系统配置）',
        http1: 'HTTP/1.1',
        http2: 'HTTP/2',
        guideTitle: '运营选择指南',
        scopeHint: '只影响 Sub2API → 模型上游的连接',
        useWhenLabel: '适用场景：',
        recommended: '推荐',
        currentSelection: '当前选择',
        recommendedAndCurrent: '推荐 · 当前选择',
        autoGuideTitle: '自动',
        autoGuideDescription: '继承系统策略：全局启用 HTTP/2 时优先使用 HTTP/2；使用代理且系统允许自动回退时，连续发生 HTTP/2 连接故障可临时回退到 HTTP/1.1。',
        autoUseWhen: '官方直连、日常运行，或尚未确认问题是否由协议导致时。',
        http1GuideTitle: 'HTTP/1.1（兼容优先）',
        http1GuideDescription: '不使用 HTTP/2 多路复用，并发请求可能建立更多 TCP/TLS 连接，但可避开部分代理、NAT 或负载均衡器的 HTTP/2 长连接兼容问题。',
        http1UseWhen: '高并发时大量请求一起卡住、首 Token 突然升至几十或上百秒，或频繁出现连接重置/超时，而官方直连稳定时，用它排查。',
        http2GuideTitle: 'HTTP/2（性能优先）',
        http2GuideDescription: '在较少 TCP/TLS 连接上多路复用请求，通常能减少连接数和握手开销；账号并发限制仍保持原值。',
        http2UseWhen: '官方直连，或已确认代理、NAT 和负载均衡器完整支持 HTTP/2 且运行稳定时。',
        decisionHint: '选择原则：高并发本身不是切换理由；稳定时保持“自动”。只有出现上述 HTTP/2 兼容症状时，才切 HTTP/1.1 排查。单独出现 connection refused 通常表示目标端口或代理拒绝连接，并不能证明是 HTTP/2 问题。',
        switchHint: '协议选择不会改变客户端请求接口、账号并发数、超时或模型能力。切换只影响新请求，正在运行的流式请求不会中断。',
