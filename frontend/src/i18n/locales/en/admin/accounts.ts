export default {
    accounts: {
      ...
      upstreamHTTPVersion: {
        ...
        description: 'Controls the HTTP protocol this account uses to connect to an OpenAI-compatible model upstream.',
        auto: 'Auto (inherit system setting)',
        http1: 'HTTP/1.1',
        http2: 'HTTP/2',
        guideTitle: 'Operations selection guide',
        scopeHint: 'Only affects the Sub2API → model upstream connection',
        useWhenLabel: 'Use when: ',
        recommended: 'Recommended',
        currentSelection: 'Current',
        recommendedAndCurrent: 'Recommended · Current',
        autoGuideTitle: 'Auto',
        autoGuideDescription: 'Inherits the system policy: prefers HTTP/2 when it is enabled globally. When a proxy is in use and automatic fallback is allowed, repeated HTTP/2 connection failures can temporarily fall back to HTTP/1.1.',
        autoUseWhen: 'Using an official direct connection, for normal operation, or when the protocol has not been confirmed as the cause.',
        http1GuideTitle: 'HTTP/1.1 (compatibility first)',
        http1GuideDescription: 'Disables HTTP/2 multiplexing. Concurrent requests may use more TCP/TLS connections, but this avoids HTTP/2 long-lived connection issues in some proxies, NATs, or load balancers.',
        http1UseWhen: 'Many requests stall together under load, time to first token jumps to tens or hundreds of seconds, or connection resets/timeouts recur while the official direct connection stays stable.',
        http2GuideTitle: 'HTTP/2 (performance first)',
        http2GuideDescription: 'Multiplexes requests over fewer TCP/TLS connections, usually reducing connection count and handshake overhead. The account concurrency limit is unchanged.',
        http2UseWhen: 'Using an official direct connection, or when the proxy, NAT, and load balancer are confirmed to support HTTP/2 reliably.',
        decisionHint: 'Rule of thumb: high concurrency alone is not a reason to switch. Stay on Auto while stable, and use HTTP/1.1 to diagnose only the HTTP/2 compatibility symptoms above. A connection refused error by itself usually means the target port or proxy rejected the connection; it does not prove an HTTP/2 issue.',
        switchHint: 'This choice does not change the client API, account concurrency, timeout, or model capabilities. A switch affects only new requests and does not interrupt in-flight streams.',
